...
def load_csv(csv_path, file_class_str):
    """
    Load a CSV file into the knowledge graph.
    
    Args:
        csv_path (str): Path to the CSV file to process
        file_class_str (str): IRI string for the class that instances should be created from
    
    Returns:
        int: Number of lines processed (excluding header)
    """
        ...
        for row in csv_reader:
            ...
            if line_count == 0:
                ...
                while i < row_count:
                    ...
                    i += 1
                line_count += 1
        print(f'Processed {line_count - 1} data lines.')
        return line_count - 1  # Return number of data lines processed (excluding header) 